...
# test-config.sh - Manual/destructive integration testing before deployment
# Usage: ./scripts/test-config.sh [component]
#
# This is NOT the static repository QA entry point. It runs Stow dry-runs
# against $HOME and may source shell configuration or start host programs.
# Use `devenv tasks run ci:all` (full) or `make ci` for repository-only QA.
# Use scripts/validate-setup.sh only after installation to inspect the host.
